...
import ir.mahdiparastesh.sexbook.jdtp.multidate.MultiDatePickerDialog.OnDateChangedListener;
...
public abstract class DayPickerView extends ListView
        implements OnScrollListener, OnDateChangedListener {
    ...
    public void goTo(MonthAdapter.CalendarDay day, boolean animate,
    ...
    public void onScroll(
        ...
        mPreviousScrollPosition = (long) view.getFirstVisiblePosition()
    ...
    private void restoreAccessibilityFocus(MonthAdapter.CalendarDay day) {
        if (day != null) for (int i = 0; i < getChildCount(); i++)
            if (getChildAt(i) instanceof MonthView &&
                    ((MonthView) getChildAt(i)).restoreAccessibilityFocus(day))
                return;
    }
    ...
    public boolean performAccessibilityAction(int action, Bundle arguments) {
        ...
        } else if (getChildAt(0) != null && getChildAt(0).getTop() >= -1) {
            day.month--;
            if (day.month == -1) {
                day.month = 11;
                day.year--;
